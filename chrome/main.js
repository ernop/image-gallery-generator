(function () {
  ...
  function galleryExtensionVersion() {
    try {
  ...
  function readStuffFromPage() {
    ...
    const ver = galleryExtensionVersion();
    const total = imageCount + videoCount;
    let galleryModeText;
    const shortVer = ver.replace(/^0\.0\./, 'v.');
    if (videoCount > 0) {
      galleryModeText = `GalleryMode WG ${shortVer}: ${total} images`;
    } else {
      galleryModeText = `GalleryMode WG ${shortVer}: ${imageCount} images`;
    }
  ...
  async function fastSaveImage() {
    ...
    try {
      ...
      let blobUrl = blobUrlCache[currentUrl];
